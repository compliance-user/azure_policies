...
def api_authenticate(username, password, api_url, project_name):
    ...
    try:
        ...
        if resp["status"] != '200':
            sys.exit('Heatstack service is down please try again after some time...!')
...
def create_template(api_url, project_id, username, token, update_flag,
    ...
    try:
        ...
        for metadata, content in zip(json_files, content_files):
            ...
            if csv_flag:
                if req_body.get("name") in input_list:
                    total_templates += 1
                else:
                    continue
            # Content Read
    ...
    try:
        ...
        for metadata in json_files:
            ...
            if csv_flag:
                if input_list and req_body.get("name") not in input_list:
                    continue
            # with open('/home/template_names.json', 'r') as t:
            #     templates = json.load(t)
            # names_only = templates.get('templates_names')
            # template_action_resource_name = req_body['recommendation']['actions'][0]['action_resource_name']
            # if template_action_resource_name not in names_only:
            #     sys.exit('Template not found for policy {}. Cancelling policy loading...!'.format(file_name))
            if scope != 'default':
                req_body['scope'] = scope
...
def create_blueprint(api_url, project_id, username, token, update_flag, json_files):
    try:
        ...
        for metadata in json_files:
            ...
            try:
                for resource in req_body['resources']:
                    ...
                    if resp["status"] == "200" and template_details['data']:
        ...
        sys.stdout.write('\nTotal Blueprints: ' + str(total_blueprints) + "\n")
